...
def perform_niraj_url_text_crawling(
    ...
    # Limit count for testing purpose.
    if max_of_urls is not None:
        data = data[:max_of_urls]
    ...
    for entry in data:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if verbose: 
                    print(f"Scraped data cleaned and stored in {out_file_path}.")
            else:
                if verbose: 
    ...
    if verbose: 
        print(f"Websites Scraped and stored in {out_file_path}")
    return data